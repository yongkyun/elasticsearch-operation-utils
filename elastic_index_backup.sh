...
## This script is tested on 1.7.5 version

## PREFIX : Index name's prefix
## CUR_ELASTIC : indexing cluster 
## OLD_ELASTIC : backup cluster  
...
INDEX_CHECK_EXIST=`curl -i -XHEAD http://$OLD_ELASTIC/$INDEX_NAME | grep HTTP | awk {'print $2'}`
if [ "$INDEX_CHECK_EXIST" == "200" ]
then
	DATE=`date +"%Y%m%d_%H%M%S"`
	echo "$DATE $INDEX_NAME is already exist at backup elasticsearch server($OLD_ELASTIC)" >> $LOGFILE
	exit
fi
...
do
	...
	then
		...
		$URL_SEND_CMD -XDELETE http://$OLD_ELASTIC/_snapshot/$REPOSITORY_NAME/$INDEX_NAME
		DATE=`date +"%Y%m%d_%H%M%S"`
...
echo "{ \"indices\": \"$INDEX_NAME\", \"index_settings\": { \"index.number_of_replicas\": 0 }, \"ignore_index_settings\": [ \"index.refresh_interval\" ] }" > $DATA_FILE
RESULT=`$URL_SEND_CMD -XPOST http://$OLD_ELASTIC/_snapshot/$REPOSITORY_NAME/$INDEX_NAME/_restore --data-binary @$DATA_FILE`
rm -f $DATA_FILE
...
$URL_SEND_CMD -XPUT http://$OLD_ELASTIC/$INDEX_NAME/_settings --data-binary @$DATA_FILE
rm -f $DATA_FILE

while [ 1 ]
do
        CNT=`$URL_SEND_CMD -s http://$OLD_ELASTIC/_cat/shards | grep $INDEX_NAME | grep -v STARTED | wc | cut -c6-7`
        DATE=`date +"%Y%m%d_%H%M%S"`
